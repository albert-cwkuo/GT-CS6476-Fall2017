...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	if ((nrhs != 2) && (nrhs != 3)) usagerr((char*)"Either two or three input arguments are required\n");
	if (mxGetClassID(prhs[0])!= mxDOUBLE_CLASS) usagerr((char*)"Image must be of type double. Create using double(imread('image name'))\n");
	...
	{
		usagerr((char*)"Field 'img' must be a greyscale (2D) or colour (3D) matrix\n");
	}
	...
	if (img_dimcount == 2)
	{
		...
		const size_t* img_dimsizes = mxGetDimensions(prhs[0]);
	...
	if (loader == NULL) usagerr((char*)"Field 'patch_size' could not be found in parms array\n");
	parms.patch_size = (unsigned short int)mxGetScalar(loader);
	if ((parms.patch_size%2)!=1) usagerr((char*)"Field 'patch_size' must be odd\n");

	loader = mxGetField(prhs[1],0,"desc_rad");
	if (loader == NULL) usagerr((char*)"Field 'desc_rad' could not be found in parms array\n");
	...
	if (loader == NULL) usagerr((char*)"Field 'nrad' could not be found in parms array\n");
	parms.nrad = (unsigned short int)mxGetScalar(loader);

	loader = mxGetField(prhs[1],0,"nang");
	if (loader == NULL) usagerr((char*)"Field 'nang' could not be found in parms array\n");
	parms.nang = (unsigned short int)mxGetScalar(loader);

	loader = mxGetField(prhs[1],0,"var_noise");
	if (loader == NULL) usagerr((char*)"Field 'var_noise' could not be found in parms array\n");
	parms.var_noise = mxGetScalar(loader);

	loader = mxGetField(prhs[1],0,"saliency_thresh");
	if (loader == NULL) usagerr((char*)"Field 'saliency_thresh' could not be found in parms array\n");
	parms.saliency_thresh = mxGetScalar(loader);
	if ((parms.saliency_thresh > 1) || (parms.saliency_thresh < 0)) usagerr((char*)"'parms.saliency_thresh' must be between 0 and 1\n");

	loader = mxGetField(prhs[1],0,"homogeneity_thresh");
	if (loader == NULL) usagerr((char*)"Field 'homogeneity_thresh' could not be found in parms array\n");
	parms.homogeneity_thresh = mxGetScalar(loader);
	if ((parms.homogeneity_thresh > 1) || (parms.homogeneity_thresh < 0)) usagerr((char*)"'parms.homogeneity_thresh' must be between 0 and 1\n");

	loader = mxGetField(prhs[1],0,"snn_thresh");
	if (loader == NULL) usagerr((char*)"Field 'snn_thresh' could not be found in parms array\n");
	parms.snn_thresh = mxGetScalar(loader);
	if ((parms.snn_thresh > 1) || (parms.snn_thresh < 0)) usagerr((char*)"'parms.snn_thresh' must be between 0 and 1\n");
	...
	if (nrhs == 3)
	{
		...
		if (min_dim != 1) usagerr((char*)"Field 'rect'/'point' must be a 1D vector\n");
		if ((max_dim == 2) || (max_dim == 4))
		{
			...
			if (max_dim == 2)
			{
				...
				if ((calcRect.xfrom < ((parms.cor_size-1)/2)) || (calcRect.yfrom < ((parms.cor_size-1)/2)) || (calcRect.xfrom > (img_width - 1 - (parms.cor_size - 1)/2)) || (calcRect.yfrom > (img_height - 1 - (parms.cor_size - 1)/2)))
					usagerr((char*)"Field 'point' is outside of allowable margin of (parms.patch_size-1)/2 + parms.desc_rad around the image\n");
			}
			if (max_dim == 4)
			{
				calcRect.xfrom = (int)rect[0] - 1;
				...
				if ((calcRect.xfrom < ((parms.cor_size-1)/2)) || (calcRect.yfrom < ((parms.cor_size-1)/2)) || (calcRect.xto > (img_width - 1 - (parms.cor_size - 1)/2)) || (calcRect.yto > (img_height - 1 - (parms.cor_size - 1)/2)))
					usagerr((char*)"Field 'rect' specifies a region outside of allowable margin of (parms.patch_size-1)/2 + parms.desc_rad around the image\n");
			}
		} else {
			usagerr((char*)"Field 'rect'/'point' must be a vector of length either 4 or 2 respectively\n");
		}
