...
fprintf('Using %s feature representation for images\n', FEATURE)
...
switch lower(FEATURE)    
    ...
    case 'bag of sift'
        ...
        if spatial_pyramid
            if ~exist('vocab_sp.mat', 'file')
                fprintf('No existing visual word vocabulary found. Computing one from training images\n')
                vocab = build_vocabulary(train_image_paths, vocab_size, spatial_pyramid);
                save('vocab_sp.mat', 'vocab')
            end
        else
        ...
        fprintf('Computing bag of sift\n')
        train_image_feats = get_bags_of_sifts(train_image_paths, soft_assignment, spatial_pyramid);
        test_image_feats  = get_bags_of_sifts(test_image_paths, soft_assignment, spatial_pyramid);
    ...
    case 'combination'
        ...
        fprintf('Computing bag of sift\n')
        train_image_feats_sift = get_bags_of_sifts(train_image_paths, soft_assignment, spatial_pyramid);
        test_image_feats_sift  = get_bags_of_sifts(test_image_paths, soft_assignment, spatial_pyramid);
        % get gmm + fisher
        if ~exist('gmm_mean.mat', 'file') | ~exist('gmm_covar.mat', 'file') | ~exist('gmm_prior.mat', 'file')
            fprintf('No existing GMM clustering found. Computing one from training images\n')
            [gmm_mean, gmm_covar, gmm_prior] = build_gmm( train_image_paths, vocab_size );
            save('gmm_mean.mat','gmm_mean');
            save('gmm_covar.mat','gmm_covar');
            save('gmm_prior.mat','gmm_prior');
        end
        fprintf('Computing gmm+fisher\n')
        train_image_feats_fisher = get_fisher( train_image_paths );
        test_image_feats_fisher = get_fisher( test_image_paths );
        %get gist
        fprintf('Computing gist\n')
        train_image_feats_gist = get_gist(train_image_paths);
        test_image_feats_gist  = get_gist(test_image_paths);
        % concat features
        train_image_feats = horzcat(train_image_feats_sift, train_image_feats_gist, train_image_feats_fisher);
        test_image_feats = horzcat(test_image_feats_sift,test_image_feats_gist, test_image_feats_fisher);
...
switch lower(CLASSIFIER)    
    ...
    case 'support vector machine'
        ...
        linear = true;
        retrain = false;
        predicted_categories = svm_classify(train_image_feats, train_labels, test_image_feats, linear, retrain);
